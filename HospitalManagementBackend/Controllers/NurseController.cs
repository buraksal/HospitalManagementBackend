...
namespace HospitalManagementBackend.Controllers
{
    ...
    public class NurseController : Controller
    {
        ...
        [HttpPost]
        [Route("createpatient")]
        public IActionResult CreatePatient(NurseRequest request)
        {
            using (var context = new HospitalManagementContext())
            {
        ...
        [HttpPost]
        [Route("getPatient")]
        public Patient GetPatient(NurseRequest request)
        {
            using (var context = new HospitalManagementContext())
            {
                var patientList = (from patient in context.Patients orderby patient.Ssn select patient).ToList<Patient>();
        ...
        [HttpPut]
        [Route("updatePatient")]
        public IActionResult UpdatePatient(NurseRequest request)
        {
            using (var context = new HospitalManagementContext())
            {
                var patientList = (from patient in context.Patients orderby patient.Ssn select patient).ToList<Patient>();
        ...
        [HttpDelete]
        [Route("deletePatient")]
        public IActionResult DeletePatient(NurseRequest request)
        {
            using (var context = new HospitalManagementContext())
            {
                var patientList = (from patient in context.Patients orderby patient.Ssn select patient).ToList<Patient>();
